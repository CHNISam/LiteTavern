        ...
        {loading ? (
          ...
          <div className="character-editor-content">
            ...
            <div className="editor-section">
              ...
              /></label>
              <label>角色描述<textarea value={model.description} rows={4} onChange={(event) => field('description', event.target.value)} /></label>
              <label>开场白<textarea value={model.first_message} rows={3} onChange={(event) => field('first_message', event.target.value)} /></label>
            </div>

            <details className="advanced-editor">
              <summary>高级设定</summary>
              <div className="editor-section">
                <label>人格<textarea value={model.personality} rows={4} onChange={(event) => field('personality', event.target.value)} /></label>
                <label>场景<textarea value={model.scenario} rows={4} onChange={(event) => field('scenario', event.target.value)} /></label>
                <label>示例对话<textarea value={model.example_messages} rows={5} onChange={(event) => field('example_messages', event.target.value)} /></label>
                <label>系统提示<textarea value={model.system_prompt} rows={4} onChange={(event) => field('system_prompt', event.target.value)} /></label>
                <label>后置提示<textarea value={model.post_history_instructions} rows={3} onChange={(event) => field('post_history_instructions', event.target.value)} /></label>
                <label>备选开场白（每行一条）<textarea value={model.alternate_greetings.join('\n')} rows={4} onChange={(event) => field('alternate_greetings', event.target.value.split('\n').filter(Boolean))} /></label>
                <label>标签（逗号分隔）<input value={model.tags.join(', ')} onChange={(event) => field('tags', event.target.value.split(',').map((item) => item.trim()).filter(Boolean))} /></label>
                <div className="creator-fields">
                  <label>创作者<input value={model.creator.name} onChange={(event) => setModel((current) => ({ ...current, creator: { ...current.creator, name: event.target.value } }))} /></label>
                  <label>角色版本<input value={model.creator.character_version} onChange={(event) => setModel((current) => ({ ...current, creator: { ...current.creator, character_version: event.target.value } }))} /></label>
                </div>
                <label>创作者说明<textarea value={model.creator.notes} rows={3} onChange={(event) => setModel((current) => ({ ...current, creator: { ...current.creator, notes: event.target.value } }))} /></label>
              </div>
            </details>
