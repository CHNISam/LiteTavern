...
v0.1.0 的交付范围仅包含 **Web / PWA**。桌面客户端不在本版本的开发、测试与发布范围内；在 Web 体验达到版本验收标准前，不恢复 Desktop 路线。

根目录的 npm workspace 与默认 `dev`、`build`、`test`、`typecheck`、`lint`、`check`
命令均显式限定为 Web 客户端；发布工作流也只打包 `apps/web/dist`。Desktop 源码以后可以作为
独立、显式启用的应用恢复，但不会通过环境变量混入 Web 运行时，也不会自动进入 v0.1.0 的
安装、检查、构建或发布产物。
